...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> sendOtp() async {
    ...
    try {
      await supabase.auth.signInWithOtp(phone: number.startsWith('+') ? number : '+91$number');
      setState(() {
        otpSent = true;
  ...
  Future<void> verifyOtp() async {
    ...
    try {
      ...
      await supabase.auth.verifyOTP(
        phone: number.startsWith('+') ? number : '+91$number',
        token: otp.text.trim(),
        type: OtpType.sms,
      );
    } catch (e) {
...
Future<String?> firstBusinessId() async {
  ...
  final row = await supabase.from('businesses').select('id').eq('owner_id', uid).limit(1).maybeSingle();
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<String?>(
      ...
      builder: (context, snapshot) {
        ...
        if (businessId == null) {
          return const Center(child: Text('No business found. Complete business setup.'));
        }
            ...
            return SafeArea(
              child: ListView(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(child: statCard('Total Sales', '₹${sales.toStringAsFixed(2)}')),
                      const SizedBox(width: 12),
                      Expanded(child: statCard('Receivable', '₹${due.toStringAsFixed(2)}')),
                    ],
                  ),
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 12),
                      Expanded(child: statCard('Customers', 'Open Customers')),
                    ],
                  ),
...
class _CustomersPageState extends State<CustomersPage> {
  Future<List<Map<String, dynamic>>> load() async {
    ...
    return List<Map<String, dynamic>>.from(
      await supabase.from('customers').select().eq('business_id', id).order('name'),
    );
  }
  @override
...
class _ProductsPageState extends State<ProductsPage> {
  Future<List<Map<String, dynamic>>> load() async {
    ...
    if (id == null) return [];
    return List<Map<String, dynamic>>.from(
      await supabase.from('products').select().eq('business_id', id).order('name'),
    );
  }
  @override
  Widget build(BuildContext context) => Scaffold(
    ...
    body: FutureBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, s) {
        ...
        final rows = s.data ?? [];
        if (rows.isEmpty) return const Center(child: Text('No products yet.'));
        return ListView.builder(
          itemCount: rows.length,
          itemBuilder: (_, i) => ListTile(
...
class ReportsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => const Scaffold(
    ...
    body: Center(child: Text('Sales, profit, GST and outstanding reports will appear here.')),
  );
}
...
class _BillLine {
  ...
  final double cost;
  double qty;
  _BillLine({required this.productId, required this.name, required this.rate, required this.cost, this.qty = 1});
...
class _NewBillPageState extends State<NewBillPage> {
  ...
  Future<void> saveBill() async {
    ...
    try {
      final result = await supabase.rpc('create_invoice_transaction', params: {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Bill created: $result')));
        ...
        : ListView(
            ...
            children: [
              DropdownButtonFormField<String>(
                decoration: const InputDecoration(labelText: 'Add product', border: OutlineInputBorder()),
              ...
              ...lines.map((l) => Card(
                child: ListTile(
                  title: Text(l.name),
                  subtitle: Text('${l.qty} × ₹${l.rate.toStringAsFixed(2)}'),
